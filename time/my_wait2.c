...
struct mydev { 
	...
	unsigned char *wp;
	wait_queue_head_t readq;	//buffer empty wait queue list 
	struct cdev mycdev;
};
...
{	struct mydev *priv = filp->private_data;
	...
	if ((*pos + priv->start) == priv->end)
		return 0;

	if (0 == (priv->wp - priv->start - *pos))
		ret = wait_event_interruptible(priv->readq, (priv->wp - priv->start - *pos));
	if (ret != 0)
