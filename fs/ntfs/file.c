		...
		struct page **cached_page ,struct pagevec *lru_pvec*/)
{
	...
	do {
		...
		if (!pages[nr]) 
		{
			...
			*/
			pages[nr] = page_cache_alloc(mapping);
			ntfs_debug("after page_cache_alloc pages[0] = [%d] ",atomic_read(&(pages[0]->_count)) );
			if (unlikely(!pages[nr])) 
			{
				err = -ENOMEM;
				goto err_out;
			}
			...
			{
				err = add_to_page_cache_lru(pages[nr], mapping, index, GFP_KERNEL);
				ntfs_debug("after add_to_page_cache pages[0] = [%d] ",atomic_read(&(pages[0]->_count)) );
				if (unlikely(err)) 
				{
					if (err == -EEXIST)
					{
		...
		s64 pos, size_t bytes)
{
	...
	if (unlikely(err)) 
	{
		ntfs_error(vi->i_sb,"ntfs_attr_lookup error [%d]",err);
		if (err == -ENOENT)
			err = -EIO;
		goto err_out;
	}
		...
		loff_t pos, loff_t *ppos, size_t count)
{
	...
	read_unlock_irqrestore(&ni->size_lock, flags);
	if (pos > ll) {
		err = ntfs_attr_extend_initialized(ni, pos, &cached_page, &lru_pvec);
		if (err < 0) {
			ntfs_error(vol->sb, "Cannot perform write to inode "
					"0x%lx, attribute type 0x%x, because "
	...
	do {
		...
		size_t copied;
		ntfs_debug("new loop pos=[%lld]",pos);
		...
		status = __ntfs_grab_cache_pages(mapping, start_idx, do_pages, pages/*, &cached_page, &lru_pvec*/);
		if (unlikely(status))
			break;
		/*
		...
		if (NInoNonResident(ni)) 
		{
			status = ntfs_prepare_pages_for_non_resident_write( pages, do_pages, pos, bytes);
			if (unlikely(status)) 
			{
				loff_t i_size;
		...
		ntfs_debug("pages[0] = [%d] ",atomic_read(&(pages[0]->_count)) );
		...
		status = ntfs_commit_pages_after_write(pages, do_pages, pos, bytes);
		if (likely(!status)) {
			written += copied;
			count -= copied;
			pos += copied;
